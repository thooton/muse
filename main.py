...
def read_config():
    ...
    if os.path.exists(config_file_path):
        ...
        for key in default_config:
            default_config[key] = config.get(
                "Gemini", key, fallback=default_config[key]
            )

    if not os.path.exists(default_config["OUT_DIR"]):
        os.makedirs(default_config["OUT_DIR"])

    with open(config_file_path, "w") as configfile:
        config["Gemini"] = {key: str(default_config[key]) for key in default_config}
    ...
    return (
        ...
        float(default_config["TEMPERATURE"]),
        float(default_config["TOP_P"]),
        default_config["OUT_DIR"],
        int(default_config["COUNT_PER_FILE"]),
    )
